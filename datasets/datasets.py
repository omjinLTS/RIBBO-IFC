...
class TrajectoryDataset():
    def __init__(
        ...
        # choices are ["random", "dataset", "none"]
        ...
        if augment:
            ...
            idx = np.random.choice(
                len(self.trajectory_list), augment_num, replace=True)
            for i in trange(idx):
            ...
            print('After augmentation, len: {}'.format(
                len(self.trajectory_list)))
    ...
    def create_cache(self, search_space_id, data_dir, cache_dir, block_size=50):
        ...
        for k in k2t:
            cache_path = os.path.join(cache_dir, '{}_{}_{}.pkl'.format(
                search_space_id, k*block_size, (k+1)*block_size-1))
    ...
    def load_cache(self, search_space, cache_dir, block_size=50, n_block=None):
        ...
        if n_block is None:
            cache_files = [file for file in os.listdir(
                cache_dir) if file.startswith(search_space)]
        else:
            cache_files = ['{}_{}_{}.pkl'.format(
                search_space, i*block_size, (i+1)*block_size-1) for i in range(n_block)]
        ...
        for file in cache_files:
            ...
            with open(cache_path, 'rb') as f:
                ...
                print('Load trajectory from {}, size: {}'.format(
                    cache_path, len(t_list)))
    ...
    def get_dataset_info(self):
        ...
        for sp_id in grouped_trajectory_dict:
            ...
            y_max_mean = sum([id2info[sp_id][ds_id]['y_max']
                             for ds_id in id2info[sp_id]]) / len(id2info[sp_id])
            y_min_mean = sum([id2info[sp_id][ds_id]['y_min']
                             for ds_id in id2info[sp_id]]) / len(id2info[sp_id])
    ...
    def set_regrets(self):  # NOTE: RTG 계산
        for i in range(len(self.trajectory_list)):
            ...
            self.trajectory_list[i].regrets = metric_regret(
                self.trajectory_list[i], y_max)
    ...
    def __getitem__(self, idx):
        ...
        return {  # NOTE : items from the trajectory dataset
    ...
    def normalize_y_and_regrets(self, t):
        if self.normalize_method == "none":
            ...
            dataset_y_min, dataset_y_max = self.id2info[sp_id][
                ds_id]['y_min'], self.id2info[sp_id][ds_id]['y_max']
            span = (dataset_y_max - dataset_y_min + 1e-6) / 2.0
            l = np.random.uniform(low=dataset_y_min-span/2,
                                  high=dataset_y_min+span/2)
            h = np.random.uniform(low=dataset_y_max-span/2,
                                  high=dataset_y_max+span/2)
            scale = h-l
            if self.scale_clip_range is not None:
                scale = np.clip(
                    scale, self.scale_clip_range[0], self.scale_clip_range[1])
    ...
    def __init__(
        ...
        if prioritize:
            ...
            for i, t in enumerate(self.trajectory_list):
                sp_id, ds_id = t.metadata['search_space_id'], t.metadata['dataset_id']
                span = self.id2info[sp_id][ds_id]["y_max"] - \
                    self.id2info[sp_id][ds_id]["y_min"]
